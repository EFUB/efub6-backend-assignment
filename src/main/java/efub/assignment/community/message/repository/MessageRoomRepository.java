...
public interface MessageRoomRepository extends JpaRepository<MessageRoom, Long> {
    ...
    @Query("SELECT m FROM MessageRoom m WHERE m.post = :post AND " +
            "((m.creator = :member1 AND m.partner = :member2) OR " +
            "(m.creator = :member2 AND m.partner = :member1))")
    ...
    @Query("SELECT DISTINCT m FROM MessageRoom m " +
            "JOIN FETCH m.creator " +
            "JOIN FETCH m.partner " +
            "WHERE m.creator = :member OR m.partner = :member " +
            "ORDER BY m.createdAt DESC")
    List<MessageRoom> findAllBySenderOrReceiverWithFetchJoin(@Param("member") Member member);
}
